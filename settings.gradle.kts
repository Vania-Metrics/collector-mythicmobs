...
// never from a sibling directory. The repo is cloned into .gradle/vania-core and
// included as a composite build: "fr.samflix:vania-metrics-api" is compiled from
// its sources, at that exact ref.
//
...
rootProject.name = "collector-mythicmobs"

dependencyResolutionManagement {
    repositoriesMode = RepositoriesMode.FAIL_ON_PROJECT_REPOS
    repositories {
        // Dated snapshots (paper-api, spark-api) are read through an ivy repo, as
        // fixed versions. Through the Maven repo, Gradle treats them as SNAPSHOT, and
        // its checksum verification fails while writing verification-metadata.xml —
        // or even writes an incomplete file (gradle/gradle#32739, #26803, open). The
        // cost: no transitive dependencies. The "paper" bundle in the catalog
        // declares the ones needed.
        exclusiveContent {
            forRepository {
...
val vaniaCoreDir: File = providers.gradleProperty("vaniaCore.dir").orNull?.let { file(it) } ?: run {
    ...
    if (!dir.resolve(".git").exists()) {
        ...
        // A fetch on every build is the only way to know where the ref points TODAY,
        // whether it's a tag or a branch. Without network, keep the clone as long as
        // it's already at the right ref — otherwise refuse, rather than silently
