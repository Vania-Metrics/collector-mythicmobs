// colecteur-mythicmobs — produces VaniaMetrics-<Name>-<v>.jar in build/libs/
//
// One module = one jar, loaded by the platform if and only if the core is present
// ("depend: [VaniaMetrics]" in plugin.yml). No third-party jar is bundled: everything
// below is compileOnly.
plugins {
    java
}
...
tasks.withType<JavaCompile>().configureEach {
    // --release 21: the lobby targets Java 25, the proxy Java 21. The lower of the two wins.
    options.release = 21
...
// The jar name comes from "name:", not the main class: the server repo's module
// list refers to modules by plugin name — the same one Bukkit displays.
